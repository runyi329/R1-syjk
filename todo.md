...
## 新功能需求 - 实时动态回测效果 (2026-01-28)
- [ ] 修改后端回测算法，按天计算并推送累计数据
- [ ] 后端每完成一天的回测就返回当天的累计结果
- [ ] 前端每0.5秒显示一天的回测数据更新
- [ ] 所有指标（套利次数、收益、余额等）实时累加显示
- [ ] 回测10天需要5秒，让用户看到整个过程
- [ ] 最后停留在最终结果上

## 实时动态回测功能完成 (2026-01-28)
- [x] 修改后端回测算法，按天计算并推送累计数据
- [x] 后端每完成一天的回测就返回当天的累计结果
- [x] 前端每0.5秒显示一天的回测数据更新
- [x] 所有指标（套利次数、收益、余额等）实时累加显示
- [ ] 测试实时动态显示功能

## Bug修复 - 回测初始化卡住问题 (2026-01-28)
- [x] 诊断回测为什么卡在“正在初始化回测”阶段
- [x] 检查后端回测API是否正常执行
- [x] 检查前端轮询逻辑是否正常
- [x] 修复初始化卡住的问题 - 调整轮询间隔为1秒，添加清除进度逻辑
- [x] 修复参数默认值丢失问题 - resetParams返回参数设置页面
- [ ] 测试修复后的回测功能
